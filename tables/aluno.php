...
$sql = "SELECT a.*, c.nome as nome_curso, t.nome as nome_turma 
        FROM aluno a
...
<head>
    ...
    <title>Alunos Cadastrados - SENAI</title>
...
<body>
    <aside class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="../public/form_aluno.php" class="menu-item active"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg> Alunos</a>
    ...
    <main class="main-content">
        ...
        <div class="welcome-card" style="border-left: none;">
            <div class="header-actions">
                <h2 style="color: var(--text-primary); font-size: 24px; margin: 0;">Lista de Alunos</h2>
                <a href="../public/form_aluno.php" class="btn-primary" style="text-decoration: none; padding: 10px 16px; font-size: 14px;">+ Novo Aluno</a>
            </div>
                    ...
                    <tbody>
                        <?php if ($result->num_rows > 0): ?>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr>
                                    <td style="font-family: monospace; color: #3b82f6;"><?php echo htmlspecialchars($row['matricula']); ?></td>
                                    <td style="font-weight: 500; font-size: 15px;"><?php echo htmlspecialchars($row['nome']); ?></td>
                                    <td><?php echo htmlspecialchars($row['nome_curso'] ? $row['nome_curso'] : 'Nenhum'); ?></td>
                                    <td><?php echo htmlspecialchars($row['nome_turma'] ? $row['nome_turma'] : 'Sem turma'); ?></td>
                                    <td><?php echo date('d/m/Y', strtotime($row['data_entrada'])); ?></td>
                                    <td>
                                        <?php if (empty($row['data_saida'])): ?>
                                            <span style="color: #10b981; font-size: 13px;">Matriculado</span>
                                        <?php else: ?>
                                            <span style="color: #ef4444; font-size: 13px;">Evasão / Saída: <?php echo date('d/m/Y', strtotime($row['data_saida'])); ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div class="actions-btn">
                                            ...
                                            <a href="edit_tables/edita_aluno.php?id=<?php echo $row['id']; ?>" class="btn-icon btn-edit" title="Editar"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
                                            <form action="delete_tables/deleta_aluno.php" method="POST" style="display:inline;" onsubmit="return confirm('Tem certeza que deseja excluir o aluno <?php echo htmlspecialchars($row['nome']); ?>?');">
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="7" style="text-align: center; color: var(--text-secondary); padding: 20px;">Nenhum aluno cadastrado.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
