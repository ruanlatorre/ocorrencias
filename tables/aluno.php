                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['nome_turma'] ?? 'N/A'); ?></td>
                            <td><?php echo date('d/m/Y', strtotime($row['data_entrada'])); ?></td>
                            <td style="text-align: center; white-space: nowrap;">
                                <button onclick="window.parent.editRecord('aluno', <?php echo $row['id']; ?>)" class="btn-icon" title="Editar">
                                ...
                                <button onclick="window.parent.deleteRecord('aluno', <?php echo $row['id']; ?>)" class="btn-icon btn-icon-delete" title="Excluir">
                    ...
                    <?php endwhile; ?>
                <?php else: ?>
                    <tr><td colspan="7" style="text-align: center;">Nenhum aluno cadastrado.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
